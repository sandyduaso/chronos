...
class RetryCommand extends Command
{
    ...
    public function handle()
    {
        foreach ($this->getJobIds() as $id) {
            $job = $this->webApp['queue.failer']->find($id);
            ...
            if (is_null($job)) {
                ...
                $this->webApp['queue.failer']->forget($id);
            }
        }
    }
    ...
    protected function getJobIds()
    {
        ...
        if (count($ids) === 1 && $ids[0] === 'all') {
            $ids = Arr::pluck($this->webApp['queue.failer']->all(), 'id');
        }
    ...
    {
        $this->webApp['queue']->connection($job->connection)->pushRaw(
