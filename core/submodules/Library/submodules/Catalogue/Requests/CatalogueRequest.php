...
class CatalogueRequest extends FormRequest
{
    ...
    public function authorize()
    {
        switch ($this->method()) {
            case 'POST':
                if ($this->user()->can('store-catalogue')) {
                    return true;
                }
                break;

            case 'PUT':
                if ($this->user()->can('update-catalogue')) {
                    return true;
                }
                break;

            case 'DELETE':
                if ($this->user()->can('destroy-catalogue')) {
                    return true;
                }
                break;
    ...
    public function rules()
    {
        ...
        return [
            ...
            'code' => 'required|regex:/^[\pL\s\-\*\#\(0-9)]+$/u|unique:catalogues'.$isUpdating,
        ];
    }
