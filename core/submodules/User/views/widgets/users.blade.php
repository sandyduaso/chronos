<div class="card mb-3">
  <div class="row">
    <div class="col-sm">
      <div class="card-body">
        <h3 class="mb-1 display-1 text-primary">{{ \User\Models\User::count() }}</h3>
        <p class="m-0 h2 text-primary"><a href="{{ route('users.index') }}">{{ __('Users') }}</a></p>

        <div><a href="{{ route('users.trashed') }}" class="lead text-muted">+{{ \User\Models\User::onlyTrashed()->count() }} {{ __('deactivated') }}</a></div>
      </div>
