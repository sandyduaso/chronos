<template>
    ...
    <v-card class="elevation-0" :class="fields.toolbar.modes.distraction.model?'mode-distraction-free mb-0':'mb-3'">
        ...
        <v-card-text :class="fields.toolbar.modes.distraction.model?'pa-5 mt-5':''">
            <template v-if="!draggables.items.length">
                <v-card-text role="button" v-tooltip:top="{'html': '{{ __('Add field') }}'}" class="text-xs-center grey--text my-5" @click="addSection(draggables.items)">
                    ...
                    <p v-else class="grey--text text--lighten-1 title text-xs-center ma-0 mb-3">{{ __('No fields yet') }}</p>
                    <img width="200px" src="{{ assets('frontier/images/placeholder/empty_box.jpg') }}" alt="">
                </v-card-text>
            </template>
    ...
    <script>
        mixins.push({
            ...
            methods: {
                ...
                old () {
                    let olds = {!! json_encode(old('fields')) !!};
